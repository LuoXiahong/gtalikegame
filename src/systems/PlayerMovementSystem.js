...
// Stays under GameConfig.VEHICLE.MAX_SPEED so driving remains the faster travel
// option. Matches AISystem.js's NPC flee-speed multiplier — retune both together.
...
export const PlayerMovementSystem = {
    update(dt, entity) {
        ...
        if (entity.physics) {
            if (isMoving) {
                // velX/velY are a per-frame displacement, not a persisting velocity —
                // MovementSystem adds them to position as-is (same convention as
                // AISystem/VehiclePhysicsSystem). Must be assigned fresh, not accumulated.
